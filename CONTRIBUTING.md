...
Thanks for helping improve **oihana/php-routes**.

## Requirements

- **PHP 8.4+**
- **Composer**
- **ext-imagick** — pulled in transitively by `oihana/php-controllers`, so it
  must be available for `composer install` and the test suite to run.
- **Xdebug** or **PCOV** — only needed to measure test coverage (see below).

## Setup

```shell
git clone https://github.com/BcommeBois/oihana-php-routes.git
cd oihana-php-routes
composer install
```
